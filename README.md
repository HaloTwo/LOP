...
개인 프로젝트에서는 전투 시스템을 자유롭게 커스텀 구조로 구현할 수 있지만,  
실제 개발 환경에서는 팀 단위 규칙과 아키텍처에 맞춰 시스템을 설계하는 것이 중요하다고 판단했습니다.

따라서 임의의 전투 프레임워크를 직접 설계하기보다,  
엔진에서 공식적으로 제공하는 Gameplay Ability System(GAS)을 기반으로  
정석적인 구조와 설계 흐름을 학습하는 것을 목표로 했습니다.

GAS는 Ability / GameplayEffect / GameplayTag 기반으로  
전투 로직, 상태, 수치 처리를 명확하게 분리할 수 있어  
확장성과 유지보수성을 고려한 아키텍처 설계에 적합하다고 판단했습니다.

<details>
<summary>자세한 설계 의도 펼치기/닫기</summary>
  
### 4️⃣ 정석 아키텍처 기반 설계 경험

개인 프로젝트의 경우 자유롭게 구조를 설계할 수 있지만,  
실제 개발 환경에서는 팀 단위 아키텍처와 규칙에 맞춰  
시스템을 구현하는 것이 중요하다고 판단했습니다.

따라서 임의의 전투 시스템을 직접 설계하기보다,  
언리얼 엔진에서 공식적으로 제공하는 Gameplay Ability System(GAS)을 기반으로  
정석적인 구조와 설계 흐름을 학습하는 것을 목표로 했습니다.

특히 언리얼은 GAS, Behavior Tree, EQS 등 공식 프레임워크가 체계적으로 제공되기 때문에     
엔진 아키텍처를 이해하고 활용하는 방향으로 설계를 진행했습니다.
...
완전한 범용 구조를 목표로 하기보다,  
1인 프로젝트 환경에서 관리 가능하면서도  
지속적으로 확장 가능한 구조를 지향했습니다.
