...
AEnemyAIController::AEnemyAIController(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer.SetDefaultSubobjectClass<UCrowdFollowingComponent>("PathFollowingComponent"))
{
	...
	//Enum 형 변환 적으로
	SetGenericTeamId(static_cast<uint8>(EGameTeam::Enemy));
}
...
ETeamAttitude::Type AEnemyAIController::GetTeamAttitudeTowards(const AActor& Other) const
{
	...
	// 상대 팀이 존재하고, 그 팀 ID가 내 팀 ID보다 작으면 적대적으로 판단합니다.
...
void AEnemyAIController::OnEnemyPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	...
	if (UBlackboardComponent* BlackboardComp = GetBlackboardComponent())
	{
		...
		{
			// 타겟을 놓친 경우 → 타이머로 나중에 제거
			if (BlackboardComp->GetValueAsObject("TargetActor") == Actor)
			{
				// 타이머로 일정 시간 뒤 타겟 제거
				GetWorld()->GetTimerManager().SetTimer(LostSightTimerHandle, FTimerDelegate::CreateLambda([this]()
					{
						if (UBlackboardComponent* BBComp = GetBlackboardComponent())BBComp->ClearValue("TargetActor");
					}),
					5.f,
					false
				);
